...
etcd 默认会自动产生快照，但是这种快照只适用于本身的快速回滚，即当节点重启、异常宕机时，etcd 会优先用本地最新的 auto-snapshot 把内存索引恢复到“最近一个 checkpoint”，然后再重放少量 WAL，这样启动速度比从头扫整个 WAL 快得多。不能用于集群恢复！！

当 etcd 启动异常时可以删除最近的快照试试？

所以如果要进行 etcd 灾难恢复，必须要手动进行快照！！
