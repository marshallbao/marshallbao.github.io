...
# 只读账号
CREATE USER genaura_app_reader WITH PASSWORD 'xxx';
GRANT CONNECT ON DATABASE genaura_app TO genaura_app_reader;

\c genaura_app;
GRANT USAGE ON SCHEMA public TO genaura_app_reader;
GRANT SELECT ON ALL TABLES IN SCHEMA public TO genaura_app_reader;
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT ON TABLES TO genaura_app_reader;
...
# 修改 owner,一般用不到
# 库的 owner
ALTER DATABASE bianjie_website OWNER TO bianjie_website;

# schema 的 owner
ALTER SCHEMA public OWNER TO bianjie_website;

# 将已存在的资源修改 owner
REASSIGN OWNED BY postgres TO bianjie_website

# 表的 owner
ALTER TABLE 表名 OWNER TO bianjie_website;

# 将现有资源的权限给某个用户（空库用不到）
-- 1. 表（最重要的数据）
GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO website;

-- 2. 序列（自增主键必给，否则无法 INSERT）
GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO website;

-- 3. 函数（如果有自定义存储过程或触发器）
GRANT ALL PRIVILEGES ON ALL FUNCTIONS IN SCHEMA public TO website;

-- 4. 过程（PostgreSQL 11+ 的新对象类型）
GRANT ALL PRIVILEGES ON ALL PROCEDURES IN SCHEMA public TO website;

# 将默认权限给某个用户（只要不用其他用户创建 “对象”，这个一般用不到） 
ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT ALL ON TABLES TO bianjie_website;
