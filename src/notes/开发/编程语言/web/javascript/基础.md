...
TypeScript (TS) 是 JavaScript (JS)的一个超集，它在 JavaScript 的基础上添加了一个最核心的功能：静态类型 (Static Typing)。

在 JS 中，变量的类型在运行时才确定，可能导致一些难以追踪的错误;也就是说你得运行的时候才会发现代码有问题，而 TS 在编译时/编写时就可以发现问题，进行改正；
