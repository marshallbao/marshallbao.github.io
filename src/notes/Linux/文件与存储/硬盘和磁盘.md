...
root@inspur:/etc/rancher/k3s# pvs
  PV         VG        Fmt  Attr PSize    PFree   
  /dev/vda3  ubuntu-vg lvm2 a--  <298.00g <148.00g
root@inspur:/etc/rancher/k3s# vgs
  VG        #PV #LV #SN Attr   VSize    VFree   
  ubuntu-vg   1   2   0 wz--n- <298.00g <148.00g
root@inspur:/etc/rancher/k3s# lvs
  LV          VG        Attr       LSize   Pool Origin Data%  Meta%  Move Log Cpy%Sync Convert
  ubuntu-lv   ubuntu-vg -wi-ao---- 100.00g                                                    
  ubuntu-test ubuntu-vg -wi-a-----  50.00g   


# 创建 lv；lv名字为 ubuntu-test;vg 名字为 ubuntu-vg
lvcreate -L 50G -n ubuntu-test ubuntu-vg
...
# 格式化磁盘
mkfs.xfs /dev/mapper/ubuntu--vg-ubuntu--test

# 挂载使用
echo '/dev/mapper/ubuntu--vg-ubuntu--test /data xfs defaults 0 0' >> /etc/fstab
mount -a

# 扩容
# VG 是 PV 的集合，扩容了PV，VG 自动就库容了，所以不需要操作 VG ，直接扩容 LV 即可；
# 扩容物理分区（通过 growpart 命令，让 pv 占满了整个物理卷大小，然后通过 LV 来管理各个分区）
growpart /dev/vda 3
# 实际上这是把所有磁盘空间放在 vg 层面，然后通过 lv 来划分分区
# 如果你需要在物理分区层面来划分分区，可以通过 parted 命令来控制，主要是用于创建不同类型的物理分区（）
