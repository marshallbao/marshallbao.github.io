...
![img](ip%E5%9C%B0%E5%9D%80.assets/image-20230114182734468-1781254653923.png)
...
RFC 1918 协议把私网 IP 的范围规定得死死的，只划出了三块地：

- **A 类私网**：`10.0.0.0`~ `10.255.255.255`（只要以 `10.` 开头就是私网）
- **B 类私网**：`172.16.0.0` ~ `172.31.255.255`（必须以 `172.` 开头，且第二段在 16~31 之间）
- **C 类私网**：`192.168.0.0` ~ `192.168.255.255`（只要以 `192.168.` 开头就是私网）
...
/16的网段最好用了，不多不少
可以把
B 类私网 -->172.16.0.0/12 （254 * 254个ip）
划分成多个 /16 网段来作为 VPC 网段来维护
比如 ：
172.18.0.0/16
172.20.0.0/16
172.22.0.0/16
...
交换机那一层可以用 /24 来划分（254个ip）
172.20.2.0/24
172.20.4.0/24
172.20.6.0/24
如果太小可以把多个 /24 网段连起来(254 * n 个ip)
